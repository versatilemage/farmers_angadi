  ...
  return (
    <nav className="sticky top-0 z-50 bg-primary shadow-lg transition-all duration-300 ease-in-out backdrop-blur-md">
      ...
      {isOpen && (
        <>
          <div
            className="fixed z-10 w-full h-full min-h-screen min-w-[100vw] bg-stone-400/70"
            onClick={() => setIsOpen(false)}
          ></div>
          <div className="absolute top-[4.5em] left-0 w-full bg-gray-400 p-6 z-50 flex flex-col items-center gap-4">
            <div className="relative" ref={dropdownRef}>
              {isUserAuthenticated && (
                <div
                  className="flex items-center gap-4 w-[14rem] py-2 px-3 bg-green-900 text-white rounded-md cursor-pointer"
